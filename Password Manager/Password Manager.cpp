    ...
    if (argc > 1) {
      switch (*argv[1]) {
        case 's': {
          ...
          sql = _strdup(
              "CREATE TABLE IF NOT EXISTS Passwords(SOURCE TEXT NOT "
          ...
          Encryp(password.c_str(), key, encryptedpass);
          std::string sqstring =
          ...
          std::cout << "\n\n\n";
          Decryp(temppass, key, encryptedpass);
        ...
        case 'l': {
          ...
          while ((print = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            Decryp(temppass.c_str(), key, decryptedpass);
